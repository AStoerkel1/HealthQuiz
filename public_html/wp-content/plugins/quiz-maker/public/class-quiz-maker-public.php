...
//        }
    ...
    /*******************************************************************************************************/
        ...
        if (isset($options['enable_timer']) && $options['enable_timer'] == 'on') {
            ...
            $after_timer_text = do_shortcode(stripslashes(str_replace('%%time%%', $this->secondsToWords($options['timer']), wpautop($after_timer_text))));
            $hide_timer_cont = "";
    ...
    /*******************************************************************************************************/
        ...
        if (isset($options['limit_users']) && $options['limit_users'] == "on") {
            if($limit_users_by == 'ip'){
                $result = $this->get_user_by_ip($id, $quiz_pass_score);
            }elseif($limit_users_by == 'user_id'){
                if(is_user_logged_in()){
                    $user_id = get_current_user_id();
                    $result = $this->get_limit_user_by_id($id, $user_id, $quiz_pass_score);
                }else{
                    $result = 0;
                }
            }else{
                    ...
                    if(isset($options['active_date_pre_start_message'])){
				        $expired_quiz_message = "<div class='step active-step'>
                            <div class='ays-abs-fs'>
                                " . do_shortcode(stripslashes(wpautop($options['active_date_pre_start_message']))) . "
                            </div>
                        </div>";
                    }else{
                ...
                if (isset($options->admin_mail) && $options->admin_mail == "on") {
                    if (filter_var(get_option('admin_email'), FILTER_VALIDATE_EMAIL)) {
                        ...
                        if($send_results_admin){
                            $message_content .= $this->ays_report_mail_content($data, 'admin', $send_results_admin);
                        }
...
//                'result' => $this->ays_set_rate_id_of_result($rate_id),
    ...
    protected function get_user_by_ip($id, $quiz_pass_score){
        global $wpdb;
        $user_ip = $this->get_user_ip();
        $sql = "SELECT COUNT(*)
                FROM `{$wpdb->prefix}aysquiz_reports`
    ...
    protected function get_limit_user_by_id($quiz_id, $user_id, $quiz_pass_score){
        global $wpdb;
        $sql = "SELECT COUNT(*)
                FROM `{$wpdb->prefix}aysquiz_reports`
        ...
        if ($x === 0) {
            ...
            if($ind_leadboard_orderby == 'id'){
                if($ind_leadboard_sort == 'avg'){
                    ...
                    $sql = "SELECT DISTINCT a.user_id, a.score AS avg_score, MIN(a.duration) AS dur_avg, a.user_name, a.options
                            FROM (
                ...
                if($ind_leadboard_sort == 'avg'){
                    ...
                    $sql = "SELECT DISTINCT a.user_email, a.score AS avg_score, MIN(a.duration) AS dur_avg, a.user_id, a.user_name, a.options
                            FROM (
